...
$nid_pr=str_replace(";","",$nid_pr);
$nid_pr=str_replace("/","",$nid_pr);
$nid_pr=str_replace("è","e",$nid_pr);
$nid_pr=str_replace("à","a",$nid_pr);
$nid_pr=str_replace("ò","o",$nid_pr);
/* Nije ovaj... */
$nid_pr=str_replace("   ","-",$nid_pr);
$nid_pr=str_replace("  ","-",$nid_pr);
$nid_pr=str_replace(" ","-",$nid_pr);
...
$nid_pr2=str_replace("---","-",$nid_pr2);
$nid_pr2=str_replace("_-_","-",$nid_pr2);
$nid_pr2=str_replace("--","-",$nid_pr2);
$nid_pr2=str_replace("
","",$nid_pr2);
return $nid_pr2;
/* ovaj izgleda da jeste... */
}
...
global $conn;
// yyyyy if (get_magic_quotes_gpc()) //dali je magic quptes ukljucen
 $value = stripslashes($value); //ako je, ponisti sta je napravio
//if (!is_numeric($value)) //dali nije numericka vrjednost (dali je string?)
$value = "'" . mysqli_real_escape_string($conn, $value) . "'"; //ako je string, escapaj ga kako spada
if($sp==1)
$value=strip_tags($value);
return $value;
}
...
$pp = mysqli_query($conn, "SELECT p.*, pt.*
    ...
    WHERE pt.lang='$lang'  AND p.akt='Y' AND p.$kol=$i ORDER BY -p.pozicija DESC");
		//");WHERE pt.lang='$lang'  AND p.akt='Y' ORDER BY -p.pozicija DESC
if (mysqli_num_rows($pp)>0) {
$g=0;
echo "<div class='text-left'><ul>";
while(@$pp1=mysqli_fetch_array($pp)){
//if($g%2==0)
//$fr=" class='iright'"; else $fr="";
$nazi=$pp1["naslov"];
if(substr($pp1['slika'],-3)=="pdf") $tip="<i class='fal fa-file-pdf fa-2x align-middle'></i>";
if(substr($pp1['slika'],-3)=="xls") $tip="<i class='fal fa-file-excel fa-2x align-middle'></i>";
if(substr($pp1['slika'],-4)=="xlsx") $tip="<i class='fal fa-file-excel fa-2x align-middle'>";
if(substr($pp1['slika'],-3)=="doc") $tip="<i class='fal fa-file-word fa-2x align-middle'></i>";
if(substr($pp1['slika'],-4)=="docx") $tip="<i class='fal fa-file-word fa-2x align-middle'></i>";
if(substr($pp1['slika'],-3)=="zip") $tip="<i class='fal fa-file-archive fa-2x align-middle'></i>";
if(substr($pp1['slika'],-3)=="swf") $tip="<i class='fal fa-film fa-2x align-middle'></i>";
if(substr($pp1['slika'],-3)=="avi") $tip="<i class='fal fa-film fa-2x align-middle'></i>";
if(substr($pp1['slika'],-3)=="mpg") $tip="<i class='fal fa-film fa-2x align-middle'></i>";
if(substr($pp1['slika'],-3)=="wmv") $tip="<i class='fal fa-film fa-2x align-middle'></i>";
if(substr($pp1['slika'],-3)=="mp4") $tip="<i class='fal fa-film fa-2x align-middle'></i>";
if(substr($pp1['slika'],-3)=="mp3") $tip="<i class='fal fa-music fa-2x align-middle'></i>";
...
{
global $lang;
$pp=mysqli_query($conn, "SELECT * FROM files WHERE $kol='$i' AND akt='Y' ORDER BY -pozicija DESC");
if (mysqli_num_rows($pp)>0) {
$g=0;
echo "<ul class='pdf'>";
while(@$pp1=mysqli_fetch_array($pp)){
...
{ 
global $lang;
$pp=mysqli_query($conn, "SELECT * FROM files WHERE idstrane='$i' AND id_page=$idpage AND akt='Y'");
$g=0;
while(@$pp1=mysqli_fetch_array($pp)){
if($g%2==0)
...
$el=array();
$idParArr0=array();
$st=mysqli_query($conn, "SELECT * FROM kat_pro kp
INNER JOIN pro p ON p.id=kp.pro WHERE p.akt=1");
...
$ardd=array();
$bdnd=mysqli_query($conn, "SELECT id, id_parent FROM stavke");

while($brends=mysqli_fetch_assoc($bdnd)){


$ardd[$brends['id']]=$brends['id_parent'];
}
$idParArr=array();
foreach($el as $k => $v) {
if($ardd[$k]>0){
...
$result = array_merge($idParArr1, $idParArr, $idParArr0);
return $result;
}

function katProArr($idkat,$tab, $podkat=1)
{
global $conn;
if($podkat>0)
$iovo=" AND id_cat=$podkat";
else
$iovo="";

$in=explode(",",$idkat);
/*
$ss=mysqli_query($conn, "SELECT * FROM $tab WHERE id='$idkat'$iovo");
$ss1=mysqli_fetch_array($ss);
$in[]=$ss1['id'];
$sa=mysqli_query($conn, "SELECT * FROM $tab WHERE id_parent='$ss1[id]'");
while($sa1=mysqli_fetch_array($sa)){
$in[]=$sa1['id'];
$sg=mysqli_query($conn, "SELECT * FROM $tab WHERE id_parent='$sa1[id]'");
while($sg1=mysqli_fetch_array($sg)){
$in[]=$sg1['id'];
$st=mysqli_query($conn, "SELECT * FROM $tab WHERE id_parent='$sg1[id]'");
while($sg1=mysqli_fetch_array($st)){
$in[]=$st1['id'];
}
}
...
$el=array();
$st=mysqli_query($conn, "SELECT * FROM kat_pro WHERE kat IN($idkat)");
while($sg1=mysqli_fetch_array($st)){
$el[]=$sg1['pro'];
}
$vra=array();
...
$vra=array_filter($vra);
$evra=implode(",",$vra);
$evra=explode(",",$evra);
$evra=array_unique($evra);
return implode(",",$el);
}
function kategorije($idkat,$tab,$podkat)
{
global $conn;
$ss=mysqli_query($conn, "SELECT * FROM $tab WHERE id='$idkat'");
$ss1=mysqli_fetch_array($ss);
$in=$ss1['id'];
if($podkat==1){
$sa=mysqli_query($conn, "SELECT * FROM $tab WHERE id_parent='$ss1[id]'");
...
$sa=mysqli_query($conn, "SELECT * FROM kateg_proizvoda_new WHERE id_parent='$idkat'");
$c=0;
while($sa1=mysqli_fetch_array($sa)){
if($c==0) $za=""; else $za=",";
$in .="$za".$sa1['id'];
...
function roundCene($cc,$idv=1)
{
global $settings;
if($idv>0)
$cc=round($cc*$settings['evro_iznos']);
else
$cc=$cc;
return $cc;
}
function sprintCene($cc,$idv,$minus=0)
{
global $settings;
$cc=$cc-$minus;
$cc1=sprintf("%4.2f",$cc);
...
function formatCene($cc,$idv,$minus=0)
{
global $settings;
$cc=$cc-$minus;
...
function format_ceneSU($cc,$idv,$minus=0)
{
global $settings;
if($idv>0)
$cc=round($cc*$settings['evro_iznos']-$minus);
else
$cc=$cc-$minus;
$vas="";
//$cc2=sprintf("%4.2f",$cc);
...
function format_ceneS($cc,$idv,$minus=0)
{
global $settings;
if($idv>0)
$cc=round($cc*$settings['evro_iznos']-$minus);
...
function format_ceneS1($cc,$idv,$minus=0)
{
global $settings;
if($idv>0)
$cc=round($cc*$settings['evro_iznos']-$minus);
...
function format_ceneS2($cc,$idv="",$iznos_evra=0)
{
global $settings;
if($iznos_evra>0)
$iznosEvra=$iznos_evra; else
$iznosEvra=$settings['evro_iznos'];
if($idv>0)
$cc=round($cc*$iznosEvra);
 $vas=" RSD"; 
$cc1=sprintf("%4.2f",$cc);
$cc2=number_format ($cc1, 2, ",", ".");
return $cc2.$vas;
...
$sel=mysqli_query($conn, "SELECT * FROM files WHERE $kolona='$idupisa' AND jezik='$jezik'");
while($sel1=@mysqli_fetch_array($sel)){
$sl="opis";
$el=mysqli_query($conn, "SELECT * FROM files WHERE opis='$sel1[opis]'");
...
$pageURL .= "://";
if ($_SERVER["SERVER_PORT"] != "80") {
if(!mb_eregi("www",$_SERVER["SERVER_NAME"]))
...
$drop="";
$field_name=array("");
$resulta = mysqli_query($conn, "SELECT * FROM $key limit 1");
while($row=mysqli_fetch_array($resulta))
 {
$field_name=array_keys($row);
 }
 //array_shift($field_name);
$gova=mysqli_num_rows($resulta); 
$p=0;
...
{
//echo $key."===".$drop."<br>";
if(!mysqli_query($conn, "ALTER TABLE `$key` $drop")) echo mysqli_error();
}
if($row!="" and $tips=="add")
{
if(!mysqli_query($conn, "ALTER TABLE $key $row AFTER id")) echo mysqli_error();
}
if($ren!="" and $tips=="ren")
{
if(!mysqli_query($conn, "ALTER TABLE `$key` $ren")) echo mysqli_error();
}
}
}
$cur_link=curPageURL();
...
{
?>
<ul  class='ul divu' id="polje<?php echo $ul?>">
<?php 
...
if($teva=="text.php")
{
$ima=mysqli_num_rows(mysqli_query($conn, "SELECT * FROM pages_text_include WHERE  id_page='$id_page' AND include_file_$col='$value'"));
$imas="($ima)";
}else $imas="";*/
...
$ima=mysqli_query($conn, "SELECT * FROM pages_text_lang WHERE  id_text='$id_text' AND lang='$firstlang'");
...
$fajlo=substr($value, 0, -4);
$fajlo1=str_replace("-"," ",$fajlo);
echo "<li$zakla><label><input type='checkbox' name='include_".$col."[]' value='$value' checked /> $fajlo1</label></li>";
}
}
}
...
{
if (strpos($entry, '.php') == false) $prik="style='display:none;'"; else $prik="";

$fajlo=substr($entry, 0, -4);
$fajlo1=str_replace("-"," ",$fajlo);
echo "<li $prik><label><input type='checkbox' name='include_".$col."[]' value='$entry' /> $fajlo1</label></li>";
...
{
?>
<ul class='divu' id="polje<?php echo $ul ?>">
<?php 
$exo=explode(",",$zz1);
if(implode("",$exo)!="")
{
...
$ima=mysqli_num_rows(mysqli_query($conn, "SELECT * FROM pages_text_include WHERE id_tekst='$id' AND id_page='$id_page' AND include_file_$col='$value'"));
