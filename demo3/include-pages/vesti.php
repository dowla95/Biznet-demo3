...
$pte4 = mysqli_query($conn, "SELECT p.*, pt.*, p.id as id
        FROM pages_text p
        INNER JOIN pages_text_lang pt ON p.id = pt.id_text        
        WHERE p.tipus=9 AND pt.lang='$lang' AND p.youtube<>'' AND p.akt='Y' AND p.id_page='$page1[id]' $and_tekst ORDER BY p.pozicija ASC LIMIT 1");
?>
...
<?php include('middle/include/slidebar-left-vesti.php');?>
...
// pocetak detaljnog prikaza vesti
if($idup>0)
{
?>
                    <div class="blog">
...
if($brk==0) {
$pref="";
$cbr="Nema komentara, budite prvi ko će komentarisati ovau vest";
}
else {
$cbr="KOMENTAR";
$pref=$brk;
