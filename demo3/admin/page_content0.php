...
$ova1=mysqli_fetch_assoc($ova);
?>
<div class='naslov_smestaj'><h1>Upisi svih modela</h1></div>
	</div>
	<div class="col-3">
...
$fi=mysqli_query($conn, "SELECT * FROM pages_text WHERE $andidpS id>0 $plun ORDER BY pozicija ASC LIMIT $str,$ByPage1");
?>
...
if($og1['tipus']==1) $mod="Sadržaj tip 1";
elseif($og1['tipus']==2) $mod="Sadržaj tip 2";
elseif($og1['tipus']==3) $mod="Sadržaj tip 3";
elseif($og1['tipus']==4) $mod="Sadržaj tip 4";
elseif($og1['tipus']==5) $mod="Sadržaj tip 5";
else $mod="Sadržaj osnovni model";
?>
<tr id="sortid_<?php echo $og1['id']?>" style='<?php echo $ba?>'>
<td><?php echo $og1['id']?></td>
<td>
<a href="#" title="<?php echo str_replace("\"","",$zzL1["naslov"])?>"><?php echo $zzL1["naslov"]?></a>
</td>
<td>
<?php 
$cg=mysqli_query($conn, "SELECT * FROM pagel WHERE id_page='$og1[id_page]' AND lang='$firstlang'");
...
if($og1['tipus']==0) $u=""; else $u=$og1['tipus'];
?>
<td>
<a href="<?php echo $patHA?>/index.php?base=admin&page=page_edit_content<?php echo $u?>&id=<?php echo $og1['id']?>" class="edit_oglas"><i class="fas fa-pencil-alt olovcica"></i></a>&nbsp;&nbsp;
