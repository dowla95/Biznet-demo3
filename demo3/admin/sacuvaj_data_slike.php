...
if(!mysqli_query($conn, "UPDATE $_POST[tabli] SET akt='$akti', link=".safe($_POST["link"])."$poce$pozicija$subtip  WHERE id=$id")) echo mysqli_error($conn);
$la=mysqli_query($conn, "SELECT * FROM language WHERE akt='Y' order by pozicija ASC");
while($la1=mysqli_fetch_array($la))
{
$lang=$la1['jezik'];
$pripada=strtolower($_POST["pripada$lang"]);
$pripada=str_replace("Č","c",$pripada);
$pripada=str_replace("č","c",$pripada);
$pripada=str_replace("Ć","c",$pripada);
$pripada=str_replace("ć","c",$pripada);
$pripada=str_replace("Š","s",$pripada);
$pripada=str_replace("š","s",$pripada);
$pripada=str_replace("Ž","z",$pripada);
$pripada=str_replace("ž","z",$pripada);
$pripada=str_replace("Đ","dj",$pripada);
$pripada=str_replace("đ","dj",$pripada);
$pripada=str_replace(" - ","-",$pripada);
$pripada=str_replace("(","",$pripada);
$pripada=str_replace(")","",$pripada);
$pripada=str_replace("'","",$pripada);
$pripada=str_replace('"',"",$pripada);
...
$pripada=str_replace(".","",$pripada);
$pripada=str_replace(",","",$pripada);
$pripada=str_replace("_","-",$pripada);
$pripada=str_replace("=","",$pripada);
$pripada=str_replace("+","",$pripada);


$column="pripada=".safe($pripada).", alt=".safe($_POST["alt$lang"]).", title=".safe($_POST["title$lang"]).", ulink=".safe($_POST["ulink$lang"]);
