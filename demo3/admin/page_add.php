...
{
if($zz1["id"]==1 and $value=="Link stranice") $beza="disabled"; else $beza="";
if($zz1["id"]==1 or $zz1["id"]==63) $bez=" d-none"; else $bez="";
?>
<input type="text" value="<?php echo $nazivi?>" name='<?php echo $inp_niz[$key]?><?php echo $jez?>' class='selecte' <?php echo $beza?>/> 
<?php } ?>
</td>
 </tr>
 <?php 
...
?>
 <td>
 Ikona za MENI: <br /><input type='text' name='class_for_icon' class='selecte' value='<?php echo $zz1['class_for_icon']?>' />
 </td>
 <td>
 <!--
 Naziv CSS klase za SECTION: <br /><input type='text' name='class_for_icon1' class='selecte' value='<?php echo $zz1['class_for_icon1']?>' />
 -->
 </td>
...
<input type='submit' onclick="delme('<?php echo $_GET['id']?>'); return false;" name='novi_upis_change' class="float-left submit_dugmici_blue<?php echo $bez?>" value='Obriši ovu stranicu'>
</div>
<?php 
}
?>
...
if($mid_get>0)
{
$zai=mysqli_query($conn, "SELECT * FROM categories_group WHERE id=$mid_get");
...
<form method="post" action="<?php echo curPageURL()?>" style="float:right;width:480px;">
 <input type='hidden' value='2'  name='tip' />   
 <!--<div style='float:left;'><input type="checkbox" value='1' <?php echo $akti?> name='akt' /> Aktiviraj</div>-->
    
    <input type="submit" value="Upis / Izmena" name='up_iz' class='submit_dugmici_blue' style='float:right;padding:8px 20px;height:37px;margin-left: 15px;' />
    <input type='text' name='name' class='form-control' style='padding:3px;float:right;width:300px;font-size:12px;' value="<?php echo $zaiz?>"  placeholder="Upisati naziv novog modela ili promeniti postojeći" required>
<?php
//if(isset($_POST['up_iz'])) {
//    header("location: index.php?base=admin&page=page_add&id=$tz1[ide]&id_cat=$_GET[id_cat]");
//}
?>
	</form>  
</div> 
...
<td>
<?php
if($og1["akt"]==1) echo "<i class='fas fa-circle fa-2x green'></i>"; else echo "<i class='fas fa-circle fa-2x red'></i>";
?></td>
-->
<td>
<?php echo $og1["name"]?></td> 
 <td>
 <!--<a href="#inline_content2" class='inline'>Uredi model</a>-->
<a href="index.php?base=admin&page=models_page&id=<?php echo $og1['id']?>&izape=1" class="iframed">Uredi model</a>
 </td>

<td>
<a href="index.php?base=admin&page=page_add&id=<?php echo $id_get?>&id_cat=<?php echo $id_cat_get?>&mid=<?php echo $og1['id']?>" class="edit_oglas"><i class="fas fa-pencil-alt olovcica"></i></a>&nbsp;&nbsp;
