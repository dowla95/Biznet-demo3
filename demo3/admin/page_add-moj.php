...
<tr valign='top'>
<td style='width:300px;background:#fff;padding:5px;border:2px solid #ccc;overflow:hidden;'>
...
<?php 
$ftz = mysqli_query($conn, "SELECT p.*, pt.*
        FROM page p
        INNER JOIN pagel pt ON p.id = pt.id_page        
        WHERE pt.lang='$firstlang'");
   while($ftz1=mysqli_fetch_array($ftz))
     {
//mysqli_query($conn, "INSERT INTO page_settings SET id_page='$ftz1[id]', include_file_middle='$ftz1[include_file_middle]', include_file_left='$ftz1[include_file_left]', include_file_right='$ftz1[include_file_right]', include_file_footer='$ftz1[include_file_footer]', include_file_top='$ftz1[include_file_top]', include_file_vrh='$ftz1[include_file_vrh]', include_file_dole='$ftz1[include_file_dole]', grupisanje='$ftz1[grupisanje]', katu_pod='$ftz1[katu_pod]'");     
     }                      
...
<tr>
<td colspan='2'> 
<a href="<?php echo $patHA?>/index.php?base=admin&page=page_add&id=<?php echo $hz1['ide']?>" <?php echo $selet?> style="color:red;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $hz1["naziv"]?> <i class="fas fa-pencil-alt"></i></a>&nbsp; &nbsp;<a href="javascript:;" onclick="delme(<?php echo $hz1['ide'] ?>)" > <i class="fal fa-trash-alt crvena"></i></a></td></tr>
<?php 
...
<a href="javascript:;" style='display:block;margin:30px 0px 10px 0px;' onclick="geti()">SAČUVAJ POZICIJE <?php echo $zanas3?></a>
<div id="rezu"></div>
...
$zz1=mysqli_fetch_array($zz);
?>
Naziv CSS klase za MENI: <br />
<input type='text' name='class_for_icon' class='selecte' value='<?php echo $zz1['class_for_icon']?>' />
</div>
<div class="col-8">
<span>Koristi ovaj model prikaza za listanje</span>
...
<input type='submit' name='izmena_stranice' class="float-right submit_dugmici_blue" value='Izmeni stranicu' />
<?php if($zz1["id"]==1) $bez=" d-none"; else $bez=""; ?>
<input type='submit' onclick="delme('<?php echo $_GET['id']?>'); return false;" name='novi_upis_change' class="float-left submit_dugmici_blue<?php echo $bez?>" value='Obrisi stranicu'>
</div>
</div>

<!--
<table style='width:100%' class='forma-lista' cellspacing='10' cellpadding='0'>
 <tr>
<?php 
$zz=mysqli_query($conn, "SELECT * FROM page  WHERE id=$id_get");               
$zz1=mysqli_fetch_array($zz);
?>
 <td>
 Naziv CSS klase za MENI: <br /><input type='text' name='class_for_icon' class='selecte' style='width:200px;' value='<?php echo $zz1['class_for_icon']?>' />
 </td>
 <td>
...
<?php if($zz1["id"]==3 or $zz1["id"]==49 or $zz1["id"]==34 or $zz1["id"]==1) $bez=" d-none"; else $bez=""; ?>
<input type='submit' onclick="delme('<?php echo $_GET['id']?>'); return false;" name='novi_upis_change' class="float-left submit_dugmici_blue<?php echo $bez?>" value='Obrisi stranicu'>
</div>
-->
<?php 
...
$la=mysqli_query($conn, "SELECT * FROM language WHERE akt='Y' order by pozicija ASC");
 ...
 foreach($inp_niz1 as $key => $value)
 {
//  if($key<4 and $key!=1) ovako je u originalu
 if($key<2 and $key!=1)
 {
 ?>
 <tr>
