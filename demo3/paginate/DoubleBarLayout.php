...
class DoubleBarLayout implements PageLayout {
	...
	public function fetchPagedLinks($parent, $queryVars,$ijos="") {
  if($ijos!="") $ijos="?$ijos";
	echo "<ul class='paginations'>";
		...
		if(!$parent->isFirstPage()) {
			...
			$str .= "<li class='page_prev  box_border'><a href=\"$queryVars/$previousPage/$ijos\" >&laquo;</a></li>";
		}
		
		if(!$parent->isFirstPage()) {
			if($currentPage>4) {
					$str .= "<li class='page_first box_border'><a href='$queryVars/1/$ijos' title='Start' >1...</a> </li> ";
			}
		}
	...
	$str .= "<li class='page_num  box_border'><a href=\"$queryVars/$i/$ijos\"  >$i </a></li>";
			}
		...
		if (!$parent->isLastPage()) {
			...
			{
				$str .= "<li class='page_last  box_border'><a href=\"$queryVars/".$parent->fetchNumberPages()."/$ijos\" title=\"Last\" >...".$parent->fetchNumberPages()." </a></li>";
			}
		}

		if(!$parent->isLastPage()) {
			$nextPage = $currentPage + 1;
			$str .= "<li class='page_next  box_border'><a href=\"$queryVars/$nextPage/$ijos\"  >&raquo;</a></li>";
		}
		return $str;
	}
}
